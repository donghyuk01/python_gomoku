...
# 1: 흑(black), 2: 백(white) 색상 매핑
COLOR_MAP = {1: "black", 2: "white"}
...
def get_line_length(x, y, dx, dy, board):
    ...
    return (1
            + count_in_direction(x, y,  dx,  dy, board, 1)
            + count_in_direction(x, y, -dx, -dy, board, 1))
...
def count_open3(x, y, board, visited=None):
    ...
    for dx, dy in directions:
        ...
        chain_fwd = collect_chain(x, y,  dx,  dy, board, 1)
        chain_bwd = collect_chain(x, y, -dx, -dy, board, 1)
...
def count_four(x,y, board,visited=None):
    """
...
def is_forbidden(x, y, board):
    ...
    try:
        ...
        if winCheck(1, 1, board) == 1: # 흑(1) 승리 판단
            return False
...
def Score_Board():
    if Winner == None:
        turn_str = "Black" if Turn == 1 else "White"
        text_color = COLOR_MAP[Turn]
        return s.create_text(width / 2, height - Frame_Gap + 15, text = "Turn = " + turn_str, font = "Helvetica 25 bold", fill = text_color)
    else:
        s.create_rectangle(width*0.1, height/2 - 60, width*0.9, height/2 + 60, fill = "white", outline = "black", width = 3)
        win_str = "BLACK" if Winner == 1 else "WHITE"
        text_val = "Draw!" if Draw == 1 else win_str + " WINS!"
...
# Turn을 숫자로 변경 (1: 흑, 2: 백)
Turn_Num, Turn, Winner, Draw = 1, 1, None, None
...
while Winner is None and Draw is None:
    ...
    if Location_Validation():
        # 흑(1) 턴일 때 금수 확인
        if Turn == 1 and is_forbidden(X, Y, board):
            Click_Cord = [None, None]
            continue
            
        s.delete(Turn_Text)
        create_circle(Board_X1 + Board_GapX * (X - 1), Board_Y1 + Board_GapY * (Y - 1), radius=Chess_Radius, fill=COLOR_MAP[Turn])
        
        board[Y-1][X-1] = Turn
        
        # 턴 전환 및 승리 체크 변수 설정 (1 -> 2 / 2 -> 1)
        if Turn == 1:
            Turn, Colour_Check, Win_Check = 2, 1, 1
        else:
            Turn, Colour_Check, Win_Check = 1, 2, 2
