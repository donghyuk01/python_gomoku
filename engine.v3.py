...
class OmokEngine:
    ...
    def count_open3_total(self, player):
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        P = player
        E = 0
    ...
    def forbidden(self, r, c, player):
        ...
        # 보드 전체 기준으로 패턴 카운트
        open3_count = self.count_open3_total(player)
        four_count  = self.count_four_total(player)
        is_overline = self.check_overline(r, c, player)

        self.board[r, c] = 0  # 돌 제거

        if is_overline:        return True
        if open3_count >= 2:   return True
        if four_count  >= 2:   return True

        return False


    def count_four_total(self, player):
        """보드 전체 스캔으로 4 패턴 개수 반환"""
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        P = player
        E = 0
                    ...
                    for pat in four_patterns:
                        ...
                        for i in range(wlen):
                            ...
                            if not (0 <= nr < self.board_size and 0 <= nc < self.board_size):
                                ...
                                match = False
                                break
                            if self.board[nr, nc] != pat[i]:
                                match = False
                                break
                        if match:
                            key = (r, c, dr, dc, tuple(pat))
